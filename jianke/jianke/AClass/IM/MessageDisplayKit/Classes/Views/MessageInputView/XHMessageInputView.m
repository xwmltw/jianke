...
@property (nonatomic, weak) UIButton *currentSelBtn; /*!< 当前选中的按钮 */
@property (nonatomic, weak) UIButton *pictureBtn; /*!< 图片按钮 */
@property (nonatomic, weak) UIButton *cameraBtn; /*!< 相机按钮 */
@property (nonatomic, weak) UIButton *emojiBtn; /*!< 表情按钮 */
@property (nonatomic, weak) UIButton *quickAnserBtn; /*!< 快捷回复按钮 */
...
/** 创建 相片, 相机, 表情, 快捷回复, 发送 按钮 */
- (void)createBtns
{
    CGFloat btnW = (SCREEN_WIDTH - 83) * 0.25;
    CGFloat btnY = self.height - 48;
    CGFloat btnH = 48;
    
    self.pictureBtn = [self createButtonWithFrame:CGRectMake(0, btnY, btnW, btnH) image:@"v240_btn_pic" HLImage:@"v240_btn_pic_1" action:@selector(pictureBtnClick:)];
    
    self.cameraBtn = [self createButtonWithFrame:CGRectMake(btnW, btnY, btnW, btnH) image:@"v240_btn_camera" HLImage:@"v240_btn_camera_1" action:@selector(cameraBtnClick:)];
    
    self.emojiBtn = [self createButtonWithFrame:CGRectMake(btnW * 2, btnY, btnW, btnH) image:@"v240_btn_emoji" HLImage:@"v240_btn_emoji_1" action:@selector(emojiBtnClick:)];
    
    self.quickAnserBtn = [self createButtonWithFrame:CGRectMake(btnW * 3, btnY, btnW, btnH) image:@"v240_btn_quick" HLImage:@"v240_btn_quick_1" action:@selector(quickAnswerBtnClick:)];
    
    [self createSendBtnWithFrame:CGRectMake(SCREEN_WIDTH - 83, 0, 83, self.height)];
}



/** 创建按钮 */
- (UIButton *)createButtonWithFrame:(CGRect)aFrame image:(NSString *)aImage HLImage:(NSString *)aHlImage action:(SEL)aAction
{
    UIButton *btn = [[UIButton alloc] initWithFrame:aFrame];
    [btn setImage:[UIImage imageNamed:aImage] forState:UIControlStateNormal];
    [btn setImage:[UIImage imageNamed:aHlImage] forState:UIControlStateHighlighted];
    [btn setImage:[UIImage imageNamed:aHlImage] forState:UIControlStateSelected];
    [btn setBackgroundImage:[UIImage imageNamed:@"v240_btn_bg_1"] forState:UIControlStateHighlighted];
    [btn setBackgroundImage:[UIImage imageNamed:@"v240_btn_bg_1"] forState:UIControlStateSelected];
    [btn addTarget:self action:aAction forControlEvents:UIControlEventTouchUpInside];
    [self addSubview:btn];
    
    [btn mas_makeConstraints:^(MASConstraintMaker *make) {
        make.height.mas_equalTo(aFrame.size.height);
        make.width.mas_equalTo(aFrame.size.width);
        make.left.mas_equalTo(aFrame.origin.x);
...
/** 创建发送按钮 宽112, 高94 */
- (void)createSendBtnWithFrame:(CGRect)aFrame
{
    UIButton *sendBtn = [[UIButton alloc] initWithFrame:aFrame];
    [sendBtn setImage:[UIImage imageNamed:@"v240_send_-1"] forState:UIControlStateDisabled];
    [sendBtn setImage:[UIImage imageNamed:@"v240_send_1"] forState:UIControlStateHighlighted];
    [sendBtn setImage:[UIImage imageNamed:@"v240_send_0"] forState:UIControlStateNormal];
    [sendBtn setContentMode:UIViewContentModeScaleAspectFit];
...
- (void)sendBtnClick:(UIButton *)btn
{
    ...
    if ([self.delegate respondsToSelector:@selector(inputView:didClickSendBtn:)]) {
        [self.delegate inputView:self didClickSendBtn:btn];
    }
}
...
{
    DLog(@"相片按钮点击");
    [self changeCurrentSelectBtn:btn];
    if ([self.delegate respondsToSelector:@selector(inputView:didClickPictureBtn:)]) {
        [self.delegate inputView:self didClickPictureBtn:btn];
    }
}
...
- (void)cameraBtnClick:(UIButton *)btn
{
    DLog(@"相机按钮点击");
    [self changeCurrentSelectBtn:btn];
    if ([self.delegate respondsToSelector:@selector(inputView:didClickCameraBtn:)]) {
        [self.delegate inputView:self didClickCameraBtn:btn];
    }
}
...
- (void)emojiBtnClick:(UIButton *)btn
{
    DLog(@"表情按钮点击");
    [self changeCurrentSelectBtn:btn];
    if ([self.delegate respondsToSelector:@selector(inputView:didClickEmojiBtn:)]) {
        [self.delegate inputView:self didClickEmojiBtn:btn];
    }
}

/** 快捷回复按钮点击 */
- (void)quickAnswerBtnClick:(UIButton *)btn
{
    DLog(@"快捷回复按钮点击");
    [self changeCurrentSelectBtn:btn];
    if ([self.delegate respondsToSelector:@selector(inputView:didClickQuickAnswerBtn:)]) {
        [self.delegate inputView:self didClickQuickAnswerBtn:btn];
    }
}

/** 改变当前选中按钮标记 */
- (void)changeCurrentSelectBtn:(UIButton *)btn
{
    if (self.currentSelBtn != btn) {
...
//            _inputTextView.frame = CGRectMake(textViewLeftMargin, 4.5f, width, height);
            _inputTextView.frame = CGRectMake(textViewLeftMargin, 5.f, width, height);
...
{
    if ([self.delegate respondsToSelector:@selector(inputTextViewShouldEndEditing:)]) {
        [self.delegate inputTextViewShouldEndEditing:self.inputTextView];
    }
