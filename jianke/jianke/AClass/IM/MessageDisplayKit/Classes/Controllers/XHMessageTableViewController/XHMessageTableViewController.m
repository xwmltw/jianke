...
@property (nonatomic, weak, readwrite) XSJEditQuickReplyView *quickAnswerView;
@property (nonatomic, weak, readwrite) XSJPictureView *pictureView;
...
/** 添加新消息 */
- (void)addMessage:(XHMessage *)addedMessage withReload:(BOOL)isReload{
    // 判断是否是打卡应答消息
    if ([self isReportMessage:addedMessage]) {
        [self addReportMessage:addedMessage withReload:isReload];
    } else {
        [self addMessageToBehind:addedMessage withReload:isReload];
    }
}

/** 判断是否是打卡应答消息 */
- (BOOL)isReportMessage:(XHMessage *)addedMessage{
    WdMessage *message = (WdMessage *)addedMessage;
    
    if (!message) {
        return NO;
    }
    
    if (message.obj_id && [message.obj_id isKindOfClass:[ImSystemMsg class]]) {
        ImSystemMsg *imSystemMsg = message.obj_id;
        if (imSystemMsg.notice_type.integerValue == WdSystemNoticeType_JKResponseReport) {
            return YES;
        }
    }
    return NO;
...
/** 判断是否是入群/退群通知消息 */
- (BOOL)isGroupNoticeMessage:(XHMessage *)addedMessage{
    WdMessage *message = (WdMessage *)addedMessage;
    ...
    if ([message.obj_id isKindOfClass:[ImSystemMsg class]]) {
        ImSystemMsg *imSystemMsg = message.obj_id;
        if (imSystemMsg.notice_type.integerValue == WdSystemNoticeType_JoinImGroup || imSystemMsg.notice_type.integerValue == WdSystemNoticeType_quitImGroup) {
            return YES;
        }
    }
    return NO;
...
/** 添加消息到最后 */
- (void)addMessageToBehind:(XHMessage *)addedMessage withReload:(BOOL)isReload{
    WEAKSELF
    [weakSelf.messages addObject:addedMessage];
    [self judgeShowTime:YES];
    if (isReload) {
...
/** 时间是否显示 */
- (void)judgeShowTime:(BOOL)isTalking{
    
    if (self.messages.count <= 1) {
        return;
    }
    
    NSTimeInterval preStamp;
    NSTimeInterval nexStamp;
    
    XHMessage *preMessage;
    XHMessage *nextMessage;
    
    if (isTalking) {
        preMessage = [self.messages objectAtIndex:self.messages.count - 2];
        nextMessage = [self.messages lastObject];
        preStamp = preMessage.timestamp.timeIntervalSince1970;
        nexStamp = nextMessage.timestamp.timeIntervalSince1970;
        nextMessage.hiddenTime = (nexStamp - preStamp) < 300;
    }else{
        for (NSInteger index = 0; index < self.messages.count - 1; index++) {
            preMessage = self.messages[index];
            nextMessage = self.messages[index + 1];
            preStamp = preMessage.timestamp.timeIntervalSince1970;
            nexStamp = nextMessage.timestamp.timeIntervalSince1970;
            nextMessage.hiddenTime = (nexStamp - preStamp) < 300;
        }
...
/** 添加打卡消息 */
- (void)addReportMessage:(XHMessage *)addedMessage withReload:(BOOL)isReload{
    WEAKSELF
    WdMessage *message = (WdMessage *)addedMessage;
    ImSystemMsg *imSystemMsg = message.obj_id;
    NSNumber *reportId = imSystemMsg.punch_the_clock_request_id;
    NSInteger insertIndex = -1;
    
    NSInteger count = weakSelf.messages.count;
    for (NSInteger i = 0; i < count; i++) {
        WdMessage *msg = weakSelf.messages[i];
        if (((ImSystemMsg *)msg.obj_id).punch_the_clock_request_id.longLongValue != reportId.longLongValue) {
            continue;
        } else {
            insertIndex = i;
        }
    ...
    if (insertIndex >= 0) { // 找到老母了..
        // 插入数据
        [weakSelf.messages insertObject:addedMessage atIndex:insertIndex + 1];
    } else {
        // 放到末尾
        [weakSelf.messages addObject:addedMessage];
    }
...
- (void)viewDidAppear:(BOOL)animated
{
    ...
    DLog(@"=======================%@===================", NSStringFromUIEdgeInsets(self.messageTableView.contentInset));
}
...
- (XSJPictureView *)pictureView {
    if (!_pictureView) {
        XSJPictureView *pictureView = [[XSJPictureView alloc] initWithFrame:CGRectMake(0, CGRectGetHeight(self.view.bounds), CGRectGetWidth(self.view.bounds), self.keyboardViewHeight)];
        pictureView.delegate = self;
        pictureView.backgroundColor = [UIColor colorWithWhite:0.961 alpha:1.000];
...
- (XSJEditQuickReplyView*)quickAnswerView {
    if (!_quickAnswerView) {
        XSJEditQuickReplyView *quickAnswerView = [[XSJEditQuickReplyView alloc] initWithFrame:CGRectMake(0, CGRectGetHeight(self.view.bounds), CGRectGetWidth(self.view.bounds), self.keyboardViewHeight)];
        quickAnswerView.delegate = self;
        quickAnswerView.backgroundColor = [UIColor colorWithWhite:0.961 alpha:1.000];
        quickAnswerView.alpha = 0.0;
        [self.view addSubview:quickAnswerView];
        _quickAnswerView = quickAnswerView;
    }
    [self.view bringSubviewToFront:_quickAnswerView];
    return _quickAnswerView;
}
...
//    CGFloat inputViewHeight = (self.inputViewStyle == XHMessageInputViewStyleFlat) ? 45.0f : 40.0f;
    CGFloat inputViewHeight = (self.inputViewStyle == XHMessageInputViewStyleFlat) ? 94.0f : 84.0f;
...
- (void)viewDidLoad {
    ...
    self.messageTableView.contentInset = UIEdgeInsetsMake(-44, 0, 94, 0);
}
...
- (void)hideAllMenuAndTextField:(BOOL)isHideTextField
{
    // 键盘
    if (isHideTextField) {
    ...
    [UIView animateWithDuration:0.2 delay:0 options:UIViewAnimationOptionCurveEaseInOut animations:^{
        ...
        self.messageInputView.y = CGRectGetHeight(self.view.bounds) - CGRectGetHeight(self.messageInputView.frame);
        if (_pictureView) {
            self.pictureView.y = CGRectGetHeight(self.view.frame);
        }
        self.emotionManagerView.y = CGRectGetHeight(self.view.frame);
        self.quickAnswerView.y = CGRectGetHeight(self.view.frame);
        
        if ([self.messageInputView.inputTextView isFirstResponder]) {
            [self layoutAndAnimateMessageInputTextView:self.messageInputView.inputTextView];
        } else {
            
            UIEdgeInsets inset = self.messageTableView.contentInset;
            inset.bottom = self.messageInputView.height;
        ...
        [self.messageInputView deSelectBtn];
    }];
...
- (void)hideAllMenu
{
    [self hideAllMenuAndTextField:YES];
}
...
- (void)layoutOtherMenuViewHiden:(BOOL)hide {
    ...
    [UIView animateWithDuration:0.2 delay:0 options:UIViewAnimationOptionCurveEaseInOut animations:^{
        ...
        // 相片
        void (^PictureViewAnimation)(BOOL hide) = ^(BOOL hide) {
                otherMenuViewFrame = self.pictureView.frame;
                otherMenuViewFrame.origin.y = (hide ? CGRectGetHeight(self.view.frame) : (CGRectGetHeight(self.view.frame) - CGRectGetHeight(otherMenuViewFrame)));
                self.pictureView.alpha = !hide;
                self.pictureView.frame = otherMenuViewFrame;
        };
        
        
        // 快捷回复
        void (^quickAnswerViewAnimation)(BOOL hide) = ^(BOOL hide) {
                otherMenuViewFrame = self.quickAnswerView.frame;
                otherMenuViewFrame.origin.y = (hide ? CGRectGetHeight(self.view.frame) : (CGRectGetHeight(self.view.frame) - CGRectGetHeight(otherMenuViewFrame)));
                self.quickAnswerView.alpha = !hide;
                self.quickAnswerView.frame = otherMenuViewFrame;
        };
        ...
        if (hide) {
            switch (self.textViewInputViewType) {
                ...
                case XHInputViewTypePicture: {
                    PictureViewAnimation(hide);
                }
                    break;
                case XHInputViewTypeQuickAnswer: {
                    quickAnswerViewAnimation(hide);
                }
                    break;
...
- (void)inputTextViewWillBeginEditing:(XHMessageTextView *)messageInputTextView {
    ...
    [self hideAllMenuAndTextField:NO];
}
...
- (void)inputTextViewShouldEndEditing:(XHMessageTextView *)messageInputTextView
{
    [self layoutOtherMenuViewHiden:YES];
}
...
- (void)inputView:(XHMessageInputView *)inpuView didClickSendBtn:(UIButton *)sendBtn
{
    DLog(@"发送按钮点击了.......");
...
- (void)inputView:(XHMessageInputView *)inpuView didClickPictureBtn:(UIButton *)pictureBtn
{
    self.textViewInputViewType = XHInputViewTypePicture;
    
    [self layoutOtherMenuViewHiden:!pictureBtn.selected];
}


- (void)inputView:(XHMessageInputView *)inpuView didClickCameraBtn:(UIButton *)cameraBtn
{
    self.textViewInputViewType = XHInputViewTypeCamera;
    [self layoutOtherMenuViewHiden:!cameraBtn.selected];
    [self didSelecteShareMenuItem:nil atIndex:0];
    [self.messageInputView deSelectBtn];
}

- (void)inputView:(XHMessageInputView *)inpuView didClickEmojiBtn:(UIButton *)emojiBtn
{
    self.textViewInputViewType = XHInputViewTypeEmotion;
    
    [self layoutOtherMenuViewHiden:!emojiBtn.selected];
}

- (void)inputView:(XHMessageInputView *)inpuView didClickQuickAnswerBtn:(UIButton *)quickAnswerBtn
{
    self.textViewInputViewType = XHInputViewTypeQuickAnswer;
    
    [self.quickAnswerView refreshWithData];
    [self layoutOtherMenuViewHiden:!quickAnswerBtn.selected];
}
...
- (void)changeSendBtnState
{
    if (self.messageInputView.inputTextView.text.length) {
        self.messageInputView.sendBtn.enabled = YES;
    } else {
        self.messageInputView.sendBtn.enabled = NO;
    }
//    self.messageInputView.sendBtn.enabled = self.messageInputView.inputTextView.text.length;
}
    ...
    switch (index) {
        case 0: {       //相机
            ...
            [UIDeviceHardware getCameraAuthorization:^(id obj) {
                [weakSelf.photographyHelper showOnPickerViewControllerSourceType:UIImagePickerControllerSourceTypeCamera onViewController:self compled:PickerMediaBlock];
...
- (void)didSelecteEmotionBar:(NSInteger)section {
    if (self.messageInputView.inputTextView.text.length < 1) {
        return;
    }
    [self didSendTextAction:self.messageInputView.inputTextView.text];
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    // 如果是打卡/入群/退群消息 就显示特殊cell样式
    if ([self isReportMessage:(XHMessage *)message] || [self isGroupNoticeMessage:(XHMessage *)message]) {
        
        XHMessageTableViewReportCell *cell = [XHMessageTableViewReportCell cellWithTableView:tableView];
        cell.message = (WdMessage *)message;
    ...
    if (!messageTableViewCell) {
        ...
        [messageTableViewCell.contentView addGestureRecognizer:tap];
    }
...
#pragma mark - XSJEditQuickReplyViewDelegate
/** 快捷回复选中后的回调 */
- (void)eqr_didSelecteMsg:(NSString *)msg
{
    DLog(@"选中了===%@", msg);
    [self didSendTextAction:msg];
}
...
- (void)eqr_btnEditOnclick{
    ...
    [self hideAllMenu];
    EditQuickReply_VC* vc = [[EditQuickReply_VC alloc] init];
    [self.navigationController pushViewController:vc animated:YES];
}

#pragma mark - XSJPictureViewDelegate
- (void)pictureViewDidClickPictureBtn:(XSJPictureView *)pictureView
{
    [self didSelecteShareMenuItem:nil atIndex:1];
    [self.messageInputView deSelectBtn];
}
...
{
    if (pictures.count > 9) {
        [UIHelper toast:@"一次最多只能发送9张图片"];
        return;
    }
    
    for (XSJPictureCellModel *model in pictures) {
    ...
    [pictureView setPicturesUnSelected];
    sendBtn.enabled = NO;
}
